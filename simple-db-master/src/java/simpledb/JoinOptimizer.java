...
public class JoinOptimizer {
    ...
    public <T> Set<BitSet> enumerateSubsets(BitSet v, int size) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            for (BitSet s : els) {
                for (int k = v.nextSetBit(0); k>= 0; k = v.nextSetBit(k+1)) {
    ...
    public Vector<LogicalJoinNode> orderJoins(
    	...
    	String[] list = new String[joins.size()+1];
    	//Set<String> set = new HashSet<>();

    	for (String a: filterSelectivities.keySet()) {
    		list[i] = a;
    		i++;
    		//set.add(a);
    	}
    	...
    	for(int q = 0; q< list.length-1; q++) {
    		...
    		if(joins.get(q).p.equals(Predicate.Op.EQUALS)) {
    			
    		LogicalJoinNode l = new LogicalJoinNode();
    		l.t1Alias = list[q+1];
    		if(q+2 <list.length) {
        	l.t2Alias = list[q];
    		}else {
    			l.t2Alias = list[list.length-1];
    			l.f1PureName = joins.get(q).f1PureName;
            	l.f2PureName = joins.get(q).f2PureName;
            	l.p = joins.get(q).p;
            	//System.out.println(l.t1Alias+" " + l.t2Alias);
            	nodes.add(l);
            	break;
    		}
        	l.f1PureName = joins.get(q).f1PureName;
		...
		if(ch) {
		LogicalJoinNode la = new LogicalJoinNode();
		la.t2Alias = list[0];
		la.t1Alias = list[0];
		la.f1PureName = joins.get(0).f1PureName;
    	la.f2PureName = joins.get(0).f2PureName;
    	la.p = joins.get(0).p;
    	//System.out.println(la.t1Alias+" " + la.t2Alias);
    	nodes.add(la);
    	}
    	joins = nodes;
        // some code goes here
