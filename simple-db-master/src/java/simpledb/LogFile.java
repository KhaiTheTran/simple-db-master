...
//    int pageSize;
    ...
    */
    ...
    private void rollbackR(long tid2) throws NoSuchElementException, IOException {
    	 if(!tidToFirstLogRecord.containsKey(tid2)) return;
         long curFpoint = tidToFirstLogRecord.get(tid2);
         raf.seek(curFpoint);
         Set<Page> pSet = new HashSet<>();
         while(true) {
         	try {
					int type = raf.readInt();
					long tranid = raf.readLong();
					
					if (type == UPDATE_RECORD && tranid == tid2) {
						Page beforeP = readPageData(raf);
						if(!pSet.contains(beforeP)) {
							Database.getCatalog().getDatabaseFile(beforeP.getId().getTableId()).writePage(beforeP);
							Database.getBufferPool().discardPage(beforeP.getId());
							pSet.add(beforeP);
						}
						readPageData(raf);
					}else
					...
					if (type == CHECKPOINT_RECORD) {
						int typec = raf.readInt();
						raf.skipBytes(typec*LONG_SIZE*2);
					}
					
					
					if(tranid == tid2 && type == ABORT_RECORD) break;
    ...
    public void recover() throws IOException {
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                Set<Long> setpg = new HashSet<>();
                raf.seek(0);
                long lastcheckpoint = raf.readLong();
                if(lastcheckpoint != -1) {
                	raf.seek(lastcheckpoint);
                	raf.skipBytes(LONG_SIZE+ INT_SIZE);
                	int tranRc = raf.readInt();
                	for(int i = 0; i<tranRc; i++) {
                		long trid = raf.readLong();
                		long newStart = raf.readLong();
                		setpg.add(trid);
                		tidToFirstLogRecord.put(trid, newStart);
                	}
                ...
                while(true) {
                	try {
						...
						if(type == BEGIN_RECORD) {
							long longrecord = raf.readLong();
							tidToFirstLogRecord.put(tranId, longrecord);
							setpg.add(tranId);
						}else
